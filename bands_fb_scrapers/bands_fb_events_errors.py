...
df=pd.read_csv('~/data/scrapers/cz_bands_fb_events/bands_fb_events_clean.csv')
band_ids=df.loc[df['event_name'].isna(),'band_id'].tolist()
...
missing_events.to_sql("bands_fb_events_errors", engine, if_exists="append", index=False)
print("completed")
